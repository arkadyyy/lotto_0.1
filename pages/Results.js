        ...
        if (game === "chance") {
            setTimeout(() => {
                axios
                    ...
                    .then((res) => console.log("chance/////////////////////////////////",res.data.data.rows))
      ...
      <NavBar navigation={navigation} screenName={"aboutUs"} />
          <ScrollView >
              ...
              <View style={{flexDirection:"row",justifyContent:"space-evenly"}}>
                  ...
                  >
                        <Text style={game === "lotto" ?
                              { fontSize: EStyleSheet.value("$rem") * 25, borderBottomColor: "blue", borderBottomWidth: 1 }
                            : { fontSize: EStyleSheet.value("$rem") * 25 }
                                 ...
                                 <TouchableOpacity
                                  ...
                                  >
                        <Text style={game === "chance" ?
                              { fontSize: EStyleSheet.value("$rem") * 25, borderBottomColor: "blue", borderBottomWidth: 1 }
                            : { fontSize: EStyleSheet.value("$rem") * 25 }                        }>צ'אנס</Text>
                      </TouchableOpacity>
                  <TouchableOpacity
                      onPress={() => {
                          setGame("777")
...
<View style={{flexDirection:"column",alignItems:"center"}}>
                    ...
                    <View style={{ width: "80%", backgroundColor: "gray", height: 1 }} ></View>
                    <View style={{ width: "80%", backgroundColor: "#263742", height: EStyleSheet.value("$rem") * 25,marginTop: EStyleSheet.value("$rem") * 25,flexDirection:"row",justifyContent:"space-evenly" }} >
        ...
        style={{
          ...
          marginTop: 10,
                    ...
                    <ListItem key={index}>
                    {/* {console.log("",data)} */}
  <View style={{ marginRight: EStyleSheet.value("$rem") * 15 }}>
                        <Text style={{ fontSize: EStyleSheet.value("$rem") * 12, color: "black" }}> {hagrala[Object.keys(hagrala)[0]]}</Text>
  </View>
  <View style={{ marginRight: 40 }}>
    <Text style={{ fontSize: EStyleSheet.value("$rem") * 12, color: "black" }}> {hagrala[Object.keys(hagrala)[1]]}</Text>
                                </View>
  ...
  <View style={{flexDirection:"row" }}>
                                    {hagrala[Object.keys(hagrala)[2]].numbers.map(num =>
                                        ...
                                        style={{
                                           borderWidth: EStyleSheet.value("$rem") * 2,
                                           width: EStyleSheet.value("$rem") * 20,
